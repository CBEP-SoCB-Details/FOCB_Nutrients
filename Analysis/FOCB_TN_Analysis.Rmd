...
errors, we end up with lower estimates fof error at sites with small sample 
sizes.  W have little evidence of unequal variances at differen t sites, so this 
...
difference to teh qualitative interpretation of the data.
```{r}
...
But as we resuced the dimensionality of the smoothers, we got increasingly 
...
# Graphic Ideas
These are based on the Full Data, not the rrestricted data
## Trim Outliers
```{r}
recent_data <- recent_data %>%
  filter(tn > 0, tn < 1.5)
...
We see the change if FOCB monitoring practices in 2017.  Winter data is not
...
```{r core_months_jitter_plot_by_date, fig.height = 7, fig.width = 3}
